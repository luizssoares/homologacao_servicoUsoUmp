...
function slcProjeto() {
    ...
    arraySequence = [63, 59, 18, 7, 9, 16, 70, 21, 25, 24, 26, 23]

    for (i in arraySequence) {
        if (Now == arraySequence[i]) {
            console.log('out edition')
            att = document.createAttribute('readonly')
            vselect.setAttributeNode(att)
        }
    }
...
function slcAcao() {
    ...
    document.getElementById("slc_acao").addEventListener('change', function(){
        ...
        console.log(codAcao)
        for(i = 0; i < arrayOption.length; i++){
            var codigoAcao = (arrayOption[i].CODIGO).split('.')[0]
            //console.log(codigoAcao);
            if(arrayOption[i].NOME == codAcao && codigoAcao == document.getElementById("codProjeto").value){
                console.log("é")
...
function slcUnidade() {
    ...
    document.getElementById("slc_unidade").addEventListener('change', function () {
        ...
        console.log(codUni)
        var Aacao = (document.getElementById("codAcao").value).split('.')[1]
        for (i = 0; i < arrayOption.length; i++) {
            var codigoUnidade = (arrayOption[i].CODIGO).split('.')[1]
            if (arrayOption[i].NOME == codUni && codigoUnidade == Aacao) {
                //console.log("é")
